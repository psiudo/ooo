...
def algorithm(prob_info, timelimit=60):
    ...
    (수정됨) 모델의 고정된 크기를 사용하여 에이전트를 초기화합니다.
    """
    ...
    # --- (중요) 모델의 고정된 크기 정의 ---
    # 이 값들은 에러 메시지에서 직접 가져온, 학습 시 사용된 실제 크기입니다.
    MODEL_STATE_SIZE = 2017
    MODEL_ACTION_SIZE = 1625

    # --- 신경망 크기에 맞는 '마스터 액션 매퍼' 생성 ---
    # PolicyNetwork의 출력층 크기(1625)를 맞추기 위한 용도입니다.
    # len(ActionMapper) = 1 (WAIT) + num_cars * 2 이므로,
    # 1625 = 1 + num_cars * 2  =>  num_cars = 812
    master_problem_for_mapper = {'K': [[(0, 1), 812]]} 
    master_action_mapper = ActionMapper(master_problem_for_mapper)
    
    # 생성된 매퍼의 크기가 모델과 일치하는지 확인 (선택 사항)
    if len(master_action_mapper) != MODEL_ACTION_SIZE:
        print(f"Warning: Master action mapper size ({len(master_action_mapper)}) does not match model action size ({MODEL_ACTION_SIZE}).")


    # --- 환경 및 에이전트 초기화 ---
    # 1. 에이전트는 '고정된 최대 크기'로 생성하여 모델을 불러올 수 있도록 합니다.
    agent = DQNAgent(MODEL_STATE_SIZE, master_action_mapper, model_path=MODEL_PATH)
    
    # 2. 실제 시뮬레이션을 위한 환경과 '현재 문제에 맞는' 액션 매퍼를 에이전트에 설정합니다.
    env = ShipEnv(prob_info)
    agent.env = env
    agent.action_mapper = ActionMapper(prob_info) # legal action 체크용
    ...
    while not done:
        ...
        # 상태 벡터를 신경망 입력 크기(2017)에 맞게 패딩합니다.
